...
class IntervalCoverageAlgorithm(BaseAlgorithm):
    ...
    def execute_traced(self, input_data):
        ...
        # Sort intervals
        tracer.capture("SORT_BEGIN", {"description": "Sorting by (start ↑, end ↓)"})
        ...
        def recurse(remaining, max_end, depth, parent_id):
            ...
            if not remaining:
                ...
                tracer.capture(
                    "CALL_RETURN",
                    {"call_id": call_id, "return_value": [], "depth": depth},
                )
                return []
            ...
            tracer.capture(
                "DECISION_MADE",
                {
                    ...
                    "interval": current,
                    "decision": decision,
                    "reason": f"end={current['end']} {'<=' if is_covered else '>'} max_end={max_end}",
            ...
            child_result = recurse(remaining[1:], new_max_end, depth + 1, call_id)
        ...
        # Start recursion
        result = recurse(sorted_intervals, float("-inf"), 0, None)
